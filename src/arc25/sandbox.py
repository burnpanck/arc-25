...
async def evaluate_solution(challenge: Challenge, solution: Solution) -> ChallengeEval:
    ...
    return ChallengeEval(
        ...
        **{f"{k}_eval":tuple(v) for k,v in result.items()},
        ...
        example_match = float(np.mean([v.full_match for v in allv if v.full_match is not None])),
        pixel_match = float(np.mean([v.pixel_match for v in allv if v.pixel_match is not None])),
    )
